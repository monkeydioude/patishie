...
async fn update_channel(
    ...
    let parsed_result = match source_type {
        SourceType::RSSFeed => get_cookies_from_rss(&channel_name, channel_id, log_id).await.unwrap_or_default(),
        SourceType::Bakery => get_cookies_from_bakery(&settings.api_path, &channel_name, log_id).await.unwrap_or_default(),
        SourceType::Other => {
    ...
    if parsed_result.is_empty() {
        success = false;
        println!("[{}] ({}) source_type: {}, channel_name: {}, channel_id: {} - no articles found", log_id, Utc::now().timestamp_millis(), source_type, channel_name, channel_id);
    } else {
        db_bag.timers_coll
